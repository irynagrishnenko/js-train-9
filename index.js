...
function findIndexByProperty(arr, name) {
  ...
  let result = arr.findIndex((obj) => obj.name === name);
  if (result) {
    return result;
  }
  return -1;
...
function findLastEvenNumber(arr) {
  ...
  let result = arr.findLast((num) => num % 2 === 0);
  if (result) {
    return result;
  }
...
function getSubArrayAndConvertToString(arr, startIdx) {
  ...
  let sliceArray = arr.slice(startIdx);
...
function flattenSquareAndFilter(arr, threshold) {
  ...
  let mapArr = flatArr.map((elem) => elem * elem);
  return mapArr.filter((elem) => elem > threshold);
}
...
function getLastElementsSumAndPrint(arr, n) {
  ...
  let sliceArray = arr.slice(-n);
  let sum = sliceArray.reduceRight(
    (accumulator, currentValue) => accumulator + currentValue
  );
  sliceArray.forEach((elem) => console.log(elem));
  return sum;
}
...
function expandArrayByN(arr, n) {
  ...
  if ((!Array.isArray(arr), typeof n !== "number")) {
    return `Вхідний параметр має бути масивом, другий - числом`;
  }
  return arr.flatMap((elem) => Array(n).fill(elem));
}
...
function findDuplicateElements(arr) {
  ...
  let filterArray = arr.filter((elem, index) => {
...
function capitalizeWords(arr) {
  ...
  let wordsArr = arr.map((elem) => {
    return elem.at(0).toUpperCase() + elem.slice(1);
  });
  return wordsArr;
}
